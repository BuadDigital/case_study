...
public sealed class KeyEnvelopesService : IKeyEnvelopesService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (status is not (KeyAssignmentStatuses.Matched or KeyAssignmentStatuses.Unmatched))
            return (null, "حالة الإسناد يجب أن تكون matched أو unmatched");
        ...
        AddTimeline(
            ...
            status == KeyAssignmentStatuses.Matched
                ? $"تأكيد مطابقة مفتاح الصك {deedNumber}"
                : $"تسجيل عدم مطابقة مفتاح الصك {deedNumber}",
            actorUserId,
            actorDisplayName,
            now);
        ...
        if (status == KeyAssignmentStatuses.Unmatched && unmatchedPropertyId is Guid unmatchedPid)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var fromParty = request.FromParty.Trim();
        var toParty = request.ToParty.Trim();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var handoff = await _db.KeyEnvelopeHandoffs
