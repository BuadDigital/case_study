...
public sealed class PropertyTimelineService : IPropertyTimelineService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        AddEvent(
            ...
            string.IsNullOrWhiteSpace(order.AssignmentSpecialist)
                ? null
                : $"√Œ’«∆Ì «·≈”‰«œ: {order.AssignmentSpecialist.Trim()}",
            PropertyTimelineTones.Done,
        ...
        foreach (var failure in failures)
        {
            ...
            AddEvent(
                ...
                " ”ÃÌ·  ⁄–—",
                $"{failure.Title} ó {FailureStatusLabel(failure.Status)}",
                PropertyTimelineTones.Warn,
                createdAt,
                recordedAt);
    ...
    {
        WorkflowTaskPhase.Enfath => "«·»Ì«‰«  «·√Ê·Ì… ··⁄ﬁ«—",
        WorkflowTaskPhase.Bourse => "«·„—Õ·… 2 ó »Ì«‰«  «·»Ê—’…",
        WorkflowTaskPhase.Distribution => "«·„—Õ·… 3 ó  Ê“Ì⁄ «·√ÿ—«›",
        WorkflowTaskPhase.CaseStudy => "œ—«”… Õ«·… «·⁄ﬁ«—",
        WorkflowTaskPhase.Obstruction => " ⁄–— ó »«‰ Ÿ«— «·„‘—›",
        _ => "„ﬂ „·…",
    };

    private static string PartyAssignedTitle(WorkflowTaskKind kind) => kind switch
    {
        WorkflowTaskKind.FieldInspection => " ⁄ÌÌ‰ «·„⁄«Ì‰ «·„Ìœ«‰Ì",
        WorkflowTaskKind.EngineeringSurvey => " ⁄ÌÌ‰ «·„ﬂ » «·Â‰œ”Ì",
        WorkflowTaskKind.PropertyAppraisal => " ⁄ÌÌ‰ «·„ﬁÌ¯„ «·⁄ﬁ«—Ì",
 // Legacy government-review child tasks (no longer spawned).
        WorkflowTaskKind.GovernmentReview => " ⁄ÌÌ‰ «·„—«Ã⁄ «·ÕﬂÊ„Ì",
        _ => " ⁄ÌÌ‰ ÿ—›",
    };
 ...
 // Legacy government-review submissions (product surface removed).
    ...
    {
        PropertyFailureStatus.Internal => "œ«Œ·Ì",
        PropertyFailureStatus.Review => "ﬁÌœ «·„—«Ã⁄…",
        PropertyFailureStatus.Approved => "„⁄ „œ",
        PropertyFailureStatus.Returned => "„ı⁄«œ",
        PropertyFailureStatus.Resolved => "„Õ·Ê·",
        PropertyFailureStatus.Suspended => "„⁄·ﬁ",
        _ => status,
    };
}
