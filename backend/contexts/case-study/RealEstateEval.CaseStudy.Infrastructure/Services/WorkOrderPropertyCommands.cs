...
public sealed class WorkOrderPropertyCommands : IWorkOrderPropertyCommands
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (entity is null) return (null, new Dictionary<string, string> { ["_"] = "√„— «·⁄„· €Ì— „ÊÃÊœ" });
 ...
 // Never trust client ids on insert ó draft ids make EF emit UPDATE and fail with 0 rows.
        property.Id = null;
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (existing is null) return (null, new Dictionary<string, string> { ["_"] = "«·⁄ﬁ«— €Ì— „ÊÃÊœ" });
        if (existing.IsRemoved)
            return (null, new Dictionary<string, string> { ["_"] = "·« Ì„ﬂ‰  ⁄œÌ· ⁄ﬁ«— „Õ–Ê›" });
 ...
 // Never mix contact DELETE/INSERT with property UPDATE in one SaveChanges ó
 ...
 // rewrite contacts with ExecuteDelete + insert.
        ...
        catch (DbUpdateConcurrencyException ex)
        {
            ...
            return (null, new Dictionary<string, string>
            {
                ["_"] = string.IsNullOrEmpty(kinds)
                    ? " ⁄–¯— Õ›Ÿ «·⁄ﬁ«— ó √⁄œ  Õ„Ì· «·’›Õ… ÊÕ«Ê· „—… √Œ—Ï"
                    : $" ⁄–¯— Õ›Ÿ «·⁄ﬁ«— ({kinds}) ó √⁄œ  Õ„Ì· «·’›Õ… ÊÕ«Ê· „—… √Œ—Ï",
            });
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrEmpty(trimmed) && !DocumentaryWorkflowRules.HasLocationMapUrl(trimmed))
        {
            return (null, new Dictionary<string, string>
            {
                ["locationMapUrl"] = "—«»ÿ «·„Êﬁ⁄ ÌÃ» √‰ Ì»œ√ »‹ http:// √Ê https://",
            });
        }
 ...
 // owners+shares from the transcription; ownership type is editable-derived.
        ...
        if (request.OwnershipTypeIsManual)
        {
            if (!OwnershipTypes.IsKnown(request.OwnershipType))
                return (null, new Dictionary<string, string> { ["ownershipType"] = "‰Ê⁄ „·ﬂÌ… €Ì— „⁄—Ê›" });
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (trimmedReason.Length > 500)
            return (false, "”»» «·Õ–› ÿÊÌ· Ãœ«");

        var entity = await _loader.LoadAsync(poNumber, cancellationToken);
        if (entity is null) return (false, "√„— «·⁄„· €Ì— „ÊÃÊœ");

        var prop = entity.Properties.FirstOrDefault(p => p.Id == propertyId);
        if (prop is null) return (false, "«·⁄ﬁ«— €Ì— „ÊÃÊœ");
        if (prop.IsRemoved) return (false, "«·⁄ﬁ«— „Õ–Ê› „”»ﬁ«");
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var id in active)
        {
            await _failures.ResolveAsync(
                ...
                {
                    ResolutionReason = " „  “ÊÌœ —«»ÿ „Êﬁ⁄ «·Œ—Ìÿ….",
                    ContinueInstructions = "Ì„ﬂ‰ «” ∆‰«› «·⁄„· ⁄·Ï «·⁄ﬁ«—.",
                },
                cancellationToken);
        }
    }
