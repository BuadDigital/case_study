...
public partial class PartyTaskSubmissionService : IPartyTaskSubmissionService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        (payloadJson, var sourceFingerprintSeen) = TakeSourceFingerprintSeen(task, payloadJson);
        ...
        if (!canStaffCorrectFieldInspection)
            await AlertSourceChangedSinceDownloadAsync(task, sourceFingerprintSeen, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (task.Kind == WorkflowTaskKind.FieldInspection)
            await AuditInspectionCompletionAsync(task, entity, actor, now, cancellationToken);
