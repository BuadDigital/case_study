...
> **الدور:** مدير المالية والعقود (`proc-admin` — علي الأمين) أو موظف مالي (`financial-officer`)  
...
> **نموذج الاتفاق** (دخل إنفاذ بندين + معاين فرد/شركة + مراجع حكومي): انظر [نموذج المالية المتفق عليه](./الماليه/نموذج-المالية-المتفق-عليه.md) وملف PDF في نفس المجلد — **مطبّق في الكود**.
...
```mermaid
flowchart LR
    A[مسودة] -->|رفع للمشرف| B[بانتظار اعتماد المشرف]
...
5. اضغط **إصدار الفاتورة** — يُنشأ رقم فاتورة ويُنزَّل ملف PDF تلقائياً (ويمكن إعادة التحميل بزر **تحميل PDF**).
...
| `GET /api/enfaz-billing/{po}/invoice.pdf` | تنزيل فاتورة PDF |
...
سجّل دخول بحساب **علي الأمين** (مدير المالية والعقود) وافتح **الإدارة المالية**.
