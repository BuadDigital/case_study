...
| البند المالي ودورة الصرف | **~١٠٠٪** (أ/ج) | ج١–ج٩ مبنية؛ إنشاء `DisbursementBatch` متقاعد (410) لصالح الكشوف؛ التراث `disb-req` للقراءة/الصرف فقط؛ إعادة تسمية كيانات الكشف في DB مؤجّلة |
...
| الصور (EXIF) | **~٨٥٪** | مسار المعالجة على الجهاز + `photo_metadata` — مقارنة المسافة/العلم مرحلة ٢ |
...
| ج٩ | تعميم الكشوف (أو كشوف موازية) للمعاين والمراجع + تأمين `OperationsTaskSequence` — **مُنجزة**: كشف الفوترة يقبل المعاينة/المراجع/الرفع المساحي، و`POST disbursement-batch` يرد 410، وتبويب «طلب صرف» أُزيل لصالح «كشوف الفوترة»، وعدّاد مهام تشغيلية ذري عبر upsert؛ جدول `DisbursementBatches` يبقى للتراث `disb-req` فقط |
...
استخراج EXIF قبل أي معالجة + `photo_metadata` + ضغط ١٦٠٠/٨٠٪/١MB + تحويل HEIC + استثناء المستندات من الضغط، وإصلاح ترتيب الوسم ليأتي بعد التصغير — **مُنجزة**: مسار `processEvidencePhoto` (EXIF→HEIC→ضغط→ختم) على معاينة الميدان وإثبات المفاتيح (صور)، وكيان `PhotoMetadata` في schema المرفقات، والمستندات (PDF) تبقى بلا ضغط.
