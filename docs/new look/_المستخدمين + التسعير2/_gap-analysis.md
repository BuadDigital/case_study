...
| البند المالي ودورة الصرف | **~١٠٠٪** (أ/ج) | ج١–ج٩ مبنية على المسار المعتمد؛ تنظيف مسار `DisbursementBatch` التاريخي وتسمية الكيانات مؤجّل |
| المصادقة | **~٧٠٪** | كلمة المرور والجلسات والقفل ومعدل الطلبات وHTTPS وبوابة الإنتاج — **لا OTP ولا دخول بالجوال ولا أجهزة** |
| المستخدمون | **~٧٠٪** | قائمة وإنشاء وتعديل (PATCH) وحذف وعرض وتدقيق ونموذج موحّد — **لا دعوات ولا سجل أعمال في الملف** |
...
| أ٢ | `audit_log` عام + خدمة كتابة موحّدة — **مُنجزة**: `AuditLog` + `IAuditLogWriter` + schema `audit` + واجهة قراءة. تبقّى ربط تحولات الأتعاب المالية وهو عمل مجموعة ج | «كل تحول حالة مالي ⇒ audit_log» لا استثناء · ويطلبه كل بند تال |
...
| ج٨ | بند لكل صك: فهرس فريد على `(transaction, deed, user)` + تفكيك مهام مستوى الـ PO — **مُنجزة (البنية)**: `Id` مستقل + `TransactionId`/`DeedId`/`UserId` + فهرس فريد؛ تفكيك مهام الـ PO **مؤجّل** كما في ق٧ |
| ج٩ | تعميم الكشوف (أو كشوف موازية) للمعاين والمراجع + تأمين `OperationsTaskSequence` — **مُنجزة**: كشف الفوترة يقبل المعاينة/المراجع/الرفع المساحي، وحظر طلب الصرف الجديد، وعدّاد مهام تشغيلية ذري عبر upsert |
