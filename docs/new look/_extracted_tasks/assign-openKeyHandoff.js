function openKeyHandoff(env){
    ...
    if(!deliver){
      ...
      document.getElementById('keySaveBtn').addEventListener('click',function(){
        var t=keyToday();
        env.handoffs.push({type:'receive_back', state:'completed', person:'فراس كمرين', role:'مراجع حكومي', date:t.date, letter:''});
        env.status='reviewer';
    ...
    var body='<div class="form-grid">'+
      ...
      '<div class="fld full" data-f="person" id="kh_personSelFld"><label>مستخدم معرَّف في النظام *</label><div class="sel" style="width:100%"><select id="kh_personSel" style="width:100%"><option value="">— اختر المستخدم —</option>'+ASSIGNEES.map(function(a){ return '<option>'+esc(a.name+' — '+a.role)+'</option>'; }).join('')+'</select><span class="caret"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg></span></div><span class="msg"></span></div>'+
