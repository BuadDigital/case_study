...
|---|---|---|
| التسعير (البنية) | **~٨٥٪** | البنية كاملة وتعمل — المطلوب تعديل سلوك لا بناء جديد |
| البند المالي ودورة الصرف | **~٧٥٪** | ١١ حالة و١١ انتقال والكشوف والترحيل مبنية — الناقص: الإيقاف · نطاق المشرف · وسم الخصم · مسار الموظف |
| المصادقة | **~٥٥٪** | كلمة المرور والجلسات والقفل ومعدل الطلبات ممتازة — **لا OTP ولا جوال ولا أجهزة** |
| المستخدمون | **~٣٥٪** | قائمة وإنشاء وحذف وعرض — **لا تعديل ولا دعوة ولا تدقيق**، والحقول ناقصة |
| الإعدادات العامة (٦ تبويبات) | **~٢٥٪** | التسعيرة والمحاكم موجودتان — بيانات الشركة والمقيم والأصول والاتصالات والمهل غير موجودة |
| الصور (EXIF) | **~١٥٪** | يوجد وسم على الصورة — **لا استخراج EXIF إطلاقاً** |
| العمل دون اتصال (مرحلة ٢) | **~١٠٪** | Service worker للأصول الساكنة وبانر — لا طابور ولا طبقة حفظ واحدة |

**أهم خمس مفاجآت في الفحص:**

1. **لا يمكن تعديل مستخدم اليوم** — `UsersController` يعرض `GET` و`POST` و`DELETE` فقط، بلا أي endpoint تحديث.
2. **الدور ليس عموداً** — يُشتق بمطابقة **نص المسمى الوظيفي حرفياً** (`PrototypeRoleResolver.ExactJobTitleToRoleId`)؛ أي اختلاف حرف واحد يُنتج مستخدماً بلا دور.
3. **بند «خلاف تسعير» يظهر للمالية فعلاً** — يُستثنى من توليد الكشف فقط، لكنه ظاهر في `GET /api/inspector-fees` وفي أعمدة شاشة الصرف. هذا خرق مباشر للقاعدة الصارمة رقم ١ في التوجيه §٣.
4. **HTTPS غير مُعدّ فعلياً** — `UseHttpsRedirection` وHSTS موجودان، لكن `infra/nginx.conf` عند `listen 80;` بلا شهادة ولا منفذ ٤٤٣ ولا إعداد Kestrel. شرط WebAuthn غير محقق.
5. **مهلة التسليم تُعاد حسابها عند تحديث ترويسة أمر العمل** (`WorkOrderService.cs:528-531`) — يخالف قاعدة «الجارية تحتفظ بمهلتها المحسوبة عند الاستلام».
...
| دور «منسق العمليات» | غير موجود؛ الأقرب `valuation-coordinator` = منسق عمليات التقييم |
...
| `paid` | `disbursed` |
| `suspended` | **لا مقابل** (الأقرب `ExcludedFromBatch` وهو مفهوم آخر) |
...
`suspended` كحالة حقيقية · `supervising_department` على أي سجل مالي · `discount_flags` (وسم الأخصائي الذي يحتاج اعتماد المشرف) · `paid_amount` · `pricing_table_id` · `deed_id` · `net_amount` مخزَّناً (يُحسب) · بطاقة «موقوف» في الملخص · سجل الأعمال · التبويب المالي في الملف.
...
| المطلوب | الواقع |
|---|---|
| استخراج EXIF قبل الضغط | **لا يوجد استخراج EXIF إطلاقاً** — لا مكتبة ولا كود (بحث: `exif` · `exifr` · `piexif` · `MetadataExtractor`) |
| تخزين الإحداثيات ووقت التصوير | `FileAttachment.cs:3-16` بلا lat/lng/captured_at. الإحداثيات الموجودة هي `FieldInspectionWorkspace.MapLatitude/Longitude` — من **إدخال المستخدم أو `navigator.geolocation`**، لا من الصورة |
| JPEG · ١٦٠٠px · ٨٠٪ · سقف ١MB · لا نسخة أصلية | `inspector-photo-stamp.ts:46-48` يعيد الترميز بجودة **٠٫٩٢** بالأبعاد الكاملة وبنفس نوع الملف — لا تصغير ولا سقف. السقف الوحيد: رفض ما فوق ٨MB (`inspector-photo-upload.ts:14`) |
| تحويل HEIC | **لا يوجد** — صفر نتيجة لـ `heic`/`heif`، و`GuessMimeFromFileName` بلا `.heic` |
| الشريط المطبوع: صك + إحداثيات + وقت | الوسم مبني ومربوط قبل الرفع (`inspector-photo-upload.ts:185-216`)، لكن نصه = **التاريخ والوقت + إحداثيات الدرافت فقط** (`inspector-workspace-data.ts:397-405`) — **بلا رقم صك** |

الترتيب المطلوب (EXIF ← ضغط ← وسم) غير قائم؛ الموجود خطوة واحدة فقط في المنتصف.

### العمل دون اتصال (مرحلة ٢ — لكن البنية تُهيَّأ الآن)

موجود: service worker للأصول الساكنة وصفحة offline (`apps/shell/public/sw.js`) · manifest · بانر «لا يوجد اتصال بالإنترنت — قد لا تُحفظ التغييرات حتى يعود الاتصال.» (`OfflineBanner.tsx:7`).
ناقص: IndexedDB · workbox · background sync · طابور معلّق · أيقونة مزامنة في الشريط العلوي.
**النقطة الحرجة لقاعدة التوجيه «صمّم الحفظ عبر طبقة واحدة يسهل اعتراضها»:** غير محقق — React Query للقراءة أساساً، والحفظ نداءات مباشرة متفرقة من مكوّنات وملفات lib. مسودات المعاينة تُحفظ **على الخادم** لا محلياً، والكاش `Map` في الذاكرة (`inspector-workspace-storage.ts:33,465`).

### الإعدادات العامة (٦ تبويبات §٤أ)

| التبويب | الحالة |
|---|---|
| ١ بيانات الشركة | **ناقص** — `CASE_STUDY_PROVIDER_NAME` ثابت في `case-study-form-data.ts:20` |
| ٢ المقيم المعتمد | **ناقص** — `EvaluatorWindow.tsx:855-864` يكتب «من إعدادات النظام» ويستخدم افتراضيات ثابتة |
| ٣ الهوية والأصول | **ناقص** — الختم والتوقيع مسارات ثابتة (`/case-study/ejadah-stamp.png`)، الترويسة صورة ثابتة، العلامة المائية نص `EJADAH` في HTML |
| ٤ التسعير | **موجود** (`fee-pricing`) |
| ٥ الاتصالات | **ناقص** — لا مزوّد رسائل ولا بريد؛ `INotificationService` داخلي فقط (إشعارات داخل التطبيق + SSE) |
| ٦ ضبط النظام | **جزئي** — المحاكم موجودة (`CourtsView.tsx` بتدقيق حقيقي) وأنواع التعذرات موجودة؛ **المهل والعدّادات ناقصة** |

**المهل:** القيم ٤ و١٠ hardcoded في `BusinessDueDateCalculator.cs:11-12` ومكرّرة على الواجهة في `po-intake-data.ts:86-88`. تُخزَّن على `WorkOrder.DueDateAt` (جيد) **لكن يُعاد حسابها عند تحديث الترويسة** (`WorkOrderService.cs:528-531`) — يخالف تثبيت مهلة الجارية.
...
| # | العمل | لماذا أولاً |
|---|---|---|
| أ١ | إعداد HTTPS فعلياً (Kestrel/شهادة + ٤٤٣ في nginx) والتأكد أن `login-username` معطّل إنتاجاً | الثابت الأول · شرط WebAuthn · وثغرة مسار dev |
| أ٢ | `audit_log` عام + خدمة كتابة موحّدة (تعميم نمط `CourtAuditLog`) | «كل تحول حالة مالي ⇒ audit_log» لا استثناء · ويطلبه كل بند تال |
| أ٣ | توحيد المستخدم: عمود `role` · `mobile` · `city` · `avatar` · `has_compensation` · `status` بأربع قيم · `fee_value` · بيانات فوترة للداخلي | كل شاشات §٤ تعتمد عليه |
| أ٤ | `PATCH /api/users/{id}` (غير موجود اليوم) + شاشة تعديل | لا يمكن تنفيذ v3 §٦.٢ ولا §١١ بدونه |
| أ٥ | طبقة حفظ واحدة على الواجهة (repository) تُمرّر عبرها كل الكتابات | قاعدة التوجيه للمرحلة ٢ — تكلفتها ترتفع كثيراً لاحقاً |

### ب — التسعير (تعديل سلوك على بنية قائمة — أسرع مكسب)

| # | العمل |
|---|---|
| ب١ | حذف كل قيم البذرة والاحتياط (١١ موضع runtime) + تعميم الخطأ الصريح على كل الفئات + جعل `NormalizeTiers([])` يفشل لا يبذر |
| ب٢ | إضافة `pricing_table_id` للمستهلكين الثلاثة وختمه مع المبلغ |
| ب٣ | audit + row-version على الجداول والشرائح والإسنادات + إضافتها لاختبار الـ concurrency |
| ب٤ | رفض الفئة غير الصالحة بـ ٤٠٠ + رفض النسخ عابر الفئة والمصدر المفقود |
| ب٥ | إخراج أتعاب المفاتيح من التسعيرة (بسياسة معلنة للسجلات التاريخية وأثرها على الملخص المالي) |
| ب٦ | جعل الجدول المرتبط بأطراف غير قابل للتعديل (نسخة جديدة + إعادة ربط ذرّية) |
| ب٧ | `PartyFeePricingServiceTests` من الصفر (لا يوجد اليوم) |

### ج — البند المالي

| # | العمل |
|---|---|
| ج١ | استثناء `disputed` من **كل** استعلام مالي (لا الكشف فقط) |
| ج٢ | `supervising_department` على البند + تخويل الخصم والحسم بمشرف ذلك القسم |
| ج٣ | `suspended` كحالة مستقلة عن `ExcludedFromBatch` + استثناؤها من الكشف + بطاقة وبطاقة فلتر |
| ج٤ | `discount_flags`: وسم الأخصائي ← اعتماد المشرف |
| ج٥ | مسار الموظف: خصم ⇒ جاهز مباشرة بإبلاغ، وحظر `disputed` عليه بنياناً |
| ج٦ | حوافز الموظف: جدول `flat` يديره المشرف + `incentive_suspensions` |
...
| ج٩ | تعميم الكشوف (أو كشوف موازية) للمعاين والمراجع + تأمين `OperationsTaskSequence` |

### د — المستخدمون والشاشات

`invitations` + رابط ٢٤ ساعة + شاشة وضع كلمة المرور (تُلغي الكلمة المؤقتة الحالية) · `devices` + بصمة جهاز + قاعدة الأربع ساعات · OTP ومزوّد رسائل كطبقة مستقلة · الدخول بالجوال · سجل الأعمال · التبويب المالي في الملف · تدرج أعمدة القائمة · شاشة الإعدادات (٤ تبويبات ناقصة) · المهل قابلة للتعديل مع تثبيت الجارية.

### هـ — الصور

استخراج EXIF قبل أي معالجة + `photo_metadata` + ضغط ١٦٠٠/٨٠٪/١MB + تحويل HEIC + استثناء المستندات من الضغط، وإصلاح ترتيب الوسم ليأتي بعد التصغير.
