...
| | تجريبية (جهازك) | إنتاج (السيرفر) |
| --- | --- | --- |
| `ASPNETCORE_ENVIRONMENT` | `Development` | `Production` |
| الحسابات التجريبية | تُبذر تلقائياً | **الكود يرفض الإقلاع لو فُعّلت** |
| الهجرات | تلقائية عند التشغيل | أداة `DbMigrate` وقت النشر فقط |
| قاعدة البيانات | `ree-postgres` محلي (منفذ 5433) | قرص مستقل على السيرفر |
| الأسرار | افتراضيات تطوير | عشوائية في `/app/.env` |
| كلمات المرور | يقبل الضعيفة | يرفض أي افتراضي تطويري |

الفصل مفروض بالكود لا بالاتفاق: `Program.cs` في كل خدمة يرمي استثناءً عند محاولة
بذر بيانات تجريبية أو ترحيل تلقائي في بيئة الإنتاج.
...
> بذر بيانات تجريبية في الإنتاج بتشغيل حاوية مؤقتة بوضع `Development` على قاعدته
> يتجاوز هذا الحارس. مقبول للعرض فقط، ولا يصلح لإنتاج يحمل بيانات حقيقية.
...
`sslip.io` خدمة DNS عامة تحوّل أي عنوان IP في الاسم إلى سجل A تلقائياً، فلا تحتاج
تسجيل دومين ولا ضبط أي سجل. لسيرفر `2.28.49.30` يصير الاسم:

```
2-28-49-30.sslip.io
```

استخدمه في كل مكان يطلب الدومين — Let's Encrypt تصدر له شهادة موثوقة عادية، فلا يظهر
أي تحذير في المتصفح. للانتقال لدومين حقيقي لاحقاً: أصدر شهادة جديدة، وحدّث
`TLS_*` و `PUBLIC_APP_URL` في `/app/.env` وفي أسرار GitHub.

> اعتماد على طرف ثالث: لو توقفت `sslip.io` يتعطل حل الاسم (لا الموقع نفسه، فهو يعمل
> على الـ IP مباشرة). مقبول للتجربة والعرض، والأفضل دومين مملوك للإنتاج الحقيقي.
...
الـ stack يشمل حاوية `gotenberg` (صورة `gotenberg/gotenberg:8`) تستخدمها خدمة التقييم لتحويل HTML تقرير التقييم — كما يظهر على الشاشة — إلى PDF يُفتح من رابط `…/api/valuation-reports/<رقم التقرير>.pdf?k=<مفتاح موقّع>` دون تسجيل دخول.

- لا تحتاج أسراراً ولا منافذ منشورة: داخلية فقط على شبكة compose.
- متغير `PdfRenderer__BaseUrl` في خدمة `valuation` يشير إليها (`http://gotenberg:3000` — مضبوط في `docker-compose.prod.yml`؛ لا شيء يُضاف إلى `/app/.env`).
- تعمل الحاوية بقائمة منع لكل طلبات http(s) من الصفحات المُحوَّلة (حماية من SSRF) لأن التقرير المرسل مكتفٍ بذاته (الصور والخطوط كـ data URLs).
- لو توقفت الحاوية يرجع زر «رابط PDF» بخطأ 503 فقط ولا يتأثر أي شيء آخر.
- مفتاح الرابط يُوقَّع اشتقاقاً من `JWT_SIGNING_KEY` (لا يصلح كـ JWT) ويُحترم `JWT_PREVIOUS_SIGNING_KEY` أثناء التدوير؛ الصلاحية الافتراضية 90 يوماً (`ReportPdfLinks__LifetimeDays`).
...
1. **frontend** + **test** — يشتغلان بالتوازي: فحص أنواع الـ MFEs واختبارات الواجهة من جهة، وبناء واختبار الـ backend من جهة أخرى.
