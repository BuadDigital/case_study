...
**Last updated:** 2026-09-20 (rewritten from code; supersedes the "12 tables in `public`" version)
...
| `realestate_eval_operations` | `operations` (includes D2 `OperationsTasks`) | `OperationsDbContext` — Operations API | same |
| `realestate_eval_financial` | `financial` (includes D1 `InspectorFee*` / `DisbursementBatches`), `audit.AuditLogs` (created by its standalone baseline) | `FinancialDbContext` — Financial API | same |
...
2. **Historical `case_study` D1/D2 DDL inside the financial and operations streams is raw `migrationBuilder.Sql`.** `MigrationStreamTests.ContextStreamsOnlyTouchTheirOwnSchemas` scans migrations for `schema: "…"` literals and fails on `schema: "case_study"` outside the Case Study stream, so older alters (`financial/…/20260905121830_FinancialForeignKeysAndChecks.cs`, `operations/…/20260905121820_TaskSequenceToReferenceSequencesAndChecks.cs`) and the 2026-09-20 `SET SCHEMA` relocates use SQL strings, not `schema: "case_study"` named arguments.
...
6. **D1/D2 no longer share the `case_study` schema name.** `InspectorFee*`/`DisbursementBatches` are `financial` on the financial database; `OperationsTasks` is `operations` on the operations database (relocated 2026-09-20). Query the owner database. Historical leftover-copy scripts still dump the old `case_study.*` names.
