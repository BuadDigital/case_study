...
2. **Continue A9** — Valuation report fill done 2026-08-18 (CS EF dropped from the Valuation host). Next residual reader: Failures compose cycle (do not add Failures `depends_on` Case Study). Do not open `FailuresDbContext`, `OperationsDbContext`, or `FinancialDbContext` from Case Study. Do not add Operations `depends_on` Case Study. Do not add Financial `depends_on` Case Study. Financial no longer opens Case Study EF.  
