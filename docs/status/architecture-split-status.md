...
| 3 — remove cross-schema access | **In progress.** Lookup residuals that drop a second connection are on owner HTTP APIs. Valuation report fill and the Failures CS patches moved to Case Study HTTP 2026-08-18 — no host outside Case Study opens `CaseStudyDbContext` any more. Remaining foreign contexts: messaging outboxes (D5 by design), D10 Identity read residuals. | — |
